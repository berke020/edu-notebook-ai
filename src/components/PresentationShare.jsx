...
export default function PresentationShare() {
  ...
  useEffect(() => {
    ...
    setViews(current);
    const logKey = `share_${id}_view_log`;
    const log = JSON.parse(localStorage.getItem(logKey) || '[]');
    const entry = { ts: new Date().toISOString(), ua: navigator.userAgent };
    ...
    <div className="min-h-screen bg-[#0B0F14] text-[#E6E9EF] flex flex-col">
      <div className="px-6 py-4 flex items-center justify-between border-b border-white/10">
        <div className="text-sm flex items-center gap-2">
          {data.brandName && data.brandMode === 'header' && (
            <span className="px-2 py-1 rounded-full bg-white/10 text-[10px]">{data.brandName}</span>
          )}
          <span>{data.title || 'Sunum'}</span>
        </div>
      ...
      <div className="px-6 py-2 text-[10px] text-white/60 border-b border-white/10 flex flex-wrap gap-4">
        {viewLog[0] && <span>Son görüntüleme: {new Date(viewLog[0].ts).toLocaleString('tr-TR')}</span>}
        <span>Toplam kayıt: {viewLog.length}</span>
      </div>
      <div className="flex-1 flex items-center justify-center px-8">
        <div className={`max-w-3xl w-full transition-all duration-300 relative ${data.transition === 'slide' ? 'translate-x-0' : 'opacity-100'}`}>
          {data.brandName && data.brandMode === 'watermark' && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-5xl font-bold text-white/5 rotate-[-12deg] select-none">{data.brandName}</div>
            </div>
          )}
